...
export class EmployeeCalculate extends Component {
    ...
    async calculateSalary() {
        ...
        this.setState({ loadingCalculate: false, netIncome: (Math.round(data * 100) / 100).toFixed(2) });
    }
    ...
    async getEmployee(id) {
        ...
        if (response.status === 200) {
            const data = await response.json();
            this.setState({ id: data.id, fullName: data.fullName, birthdate: data.birthdate.toString().split('T')[0], tin: data.tin, employeeTypeId: data.employeeTypeId, loading: false, loadingCalculate: false, netIncome: (0).toFixed(2) });
        }
        else {
